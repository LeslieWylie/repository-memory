...
def _tool_schema() -> list[dict[str, Any]]:
    return [
        {"name": "memory_doctor", "description": "Inspect the server-configured repository sources, adapters, freshness, and index state. Omit root unless an explicit verified Git root is required. Set local=true to avoid remote snapshot fetches.", "inputSchema": {"type": "object", "properties": {"root": {"type": "string", "description": "Optional verified Git repository root; omit to use server discovery."}, "source": {"type": "string"}, "local": {"type": "boolean"}}, "additionalProperties": False}},
