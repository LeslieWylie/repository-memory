...
def _tool_schema() -> list[dict[str, Any]]:
    return [
        {"name": "memory_doctor", "description": "Inspect the server-configured repository sources, adapters, freshness, and index state. Omit root unless an explicit verified Git root is required. Set local=true to avoid remote snapshot fetches and no_index=true for a metadata-only fast probe.", "inputSchema": {"type": "object", "properties": {"root": {"type": "string", "description": "Optional verified Git repository root; omit to use server discovery."}, "source": {"type": "string"}, "local": {"type": "boolean"}, "no_index": {"type": "boolean"}}, "additionalProperties": False}},
        ...
        {"name": "memory_search", "description": "Search repository evidence, native conversation memory, or both. Default uses a clean remote snapshot; do not set local=true unless the user explicitly requests local/offline inspection. `verified` means the citation is real; use `answerable`/`results` for claims directly supported by the returned excerpt. If answerable is empty, abstain even when verified documents exist.", "inputSchema": {"type": "object", "required": ["query"], "properties": {"root": {"type": "string", "description": "Optional verified Git repository root; omit to use server discovery."}, "source": {"type": "string"}, "query": {"type": "string"}, "limit": {"type": "integer", "minimum": 1, "maximum": 50}, "deep": {"type": "boolean"}, "local": {"type": "boolean", "description": "Only set true when the user explicitly requests the local/offline worktree."}, "scope": {"type": "string", "enum": ["repository", "memory", "all"], "default": "repository"}}, "additionalProperties": False}},
        ...
        {"name": "memory_observe", "description": "Observe durable local memory events without ranking or generating a conclusion. This is provenance, not repository evidence, and is read-only.", "inputSchema": {"type": "object", "properties": {"session_id": {"type": "string"}, "limit": {"type": "integer", "minimum": 1, "maximum": 500}}, "additionalProperties": False}},
        {"name": "memory_reflect", "description": "Generate a bounded, candidate-labelled reflection over local memory. It is a derived aid, not an accepted fact; inspect citations before reuse.", "inputSchema": {"type": "object", "properties": {"query": {"type": "string"}, "session_id": {"type": "string"}, "limit": {"type": "integer", "minimum": 1, "maximum": 20}}, "additionalProperties": False}},
    ]
